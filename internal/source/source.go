...
func (w *Workflow) Fetch(ctx context.Context, input string) (domain.RawSnapshot, error) {
	if w == nil {
		return domain.RawSnapshot{}, internalerrors.Unsupported("source workflow is not configured")
	}
	...
	for _, transport := range w.transports {
		...
		if !ok || plugin == nil {
			return domain.RawSnapshot{}, internalerrors.Unsupported(fmt.Sprintf("no plugin for source type %q", kind))
		}
	...
	return domain.RawSnapshot{}, internalerrors.Unsupported("input is not a supported source")
}
