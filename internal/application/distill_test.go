...
func TestDistillCreatesOneCrossSourceDocument(t *testing.T) {
	...
	data, err := store.ReadDocument(context.Background(), domain.SynthesizedRef(result.Filename))
	...
	if err != nil || len(state.SynthesizedDocuments) != 1 || len(state.SynthesizedDocuments[0].DerivedFrom) != 2 {
...
func TestDistillRejectsUnreadReferencesAndAllowsSkip(t *testing.T) {
	...
	provider := &fakeProvider{responses: []agent.CompletionResponse{
		toolResponse("read-one", "read_document", `{"filename":"one.md"}`),
		toolResponse("write", "write_distill", `{"title":"Shared facts","introduction":"intro","sections":[{"heading":"Facts","paragraph":"paragraph","bullets":["one","two"],"sources":[{"source_key":"https://example.test/one","kind":"raw","filename":"one.md"},{"source_key":"https://example.test/two","kind":"raw","filename":"two.md"}]}]}`),
	...
	state, _, err := store.ReadState(context.Background())
	if err != nil || len(state.SynthesizedDocuments) != 0 {
		t.Fatalf("state = %#v, %v", state, err)
	}
}
...
func TestDistillExcludesStaleSummariesAndUsesCurrentSummaryInputs(t *testing.T) {
	...
	provider := &fakeProvider{responses: []agent.CompletionResponse{
		...
		toolResponse("write", "write_distill", `{"title":"Current facts","introduction":"intro","sections":[{"heading":"Facts","paragraph":"paragraph","bullets":["new","two"],"sources":[{"source_key":"https://example.test/one","kind":"raw","filename":"one-new.md"},{"source_key":"https://example.test/two","kind":"summary","filename":"two-summary.md"}]}]}`),
	}}
	result, err := application.Distill(context.Background(), store, provider, application.DefaultSynthesisOptions(), operationOptionsFor(target))
	...
	inputs := state.SynthesizedDocuments[0].DerivedFrom
...
func TestDistillRequiresTerminalTool(t *testing.T) {
	...
	if stateErr != nil || len(state.SynthesizedDocuments) != 0 {
