use super::*;
use crate::domain::state::{TreeMetadata, TreeState, TreeStateError};
...
fn round_trip_empty_state() {
    ...
    let path = dir.path().join(".bo/state.json");
    let original = empty_state();
...
#[test]
fn round_trip_full_state_preserves_all_fields() {
    let dir = TempDir::new().unwrap();
    let path = dir.path().join(".bo/state.json");
...
fn read_returns_parse_error_on_invalid_json() {
    ...
    assert!(matches!(err, TreeStateError::Parse(_)), "got: {err}");
}
...
fn read_returns_tree_not_initialized_when_file_absent() {
    ...
    let err = read(&path).unwrap_err();
    assert!(
        matches!(err, TreeStateError::TreeNotInitialized),
        "got: {err}"
    );
}

// ── legacy manifest split-brain safety ──────────────────────────────────────

/// Stage a sibling legacy `.bo/manifest.json` (and optionally a state.json)
/// under a temp tree's infra dir.
fn stage_legacy_manifest(dir: &std::path::Path, with_state: bool) {
    let bo_dir = dir.join(".bo");
    std::fs::create_dir_all(&bo_dir).unwrap();
    if with_state {
        write(&bo_dir.join("state.json"), &sample_state()).unwrap();
    }
    std::fs::write(bo_dir.join("manifest.json"), "{}").unwrap();
}

#[test]
fn read_rejects_legacy_manifest_when_state_absent() {
    let dir = TempDir::new().unwrap();
    stage_legacy_manifest(dir.path(), false);
    let path = dir.path().join(".bo/state.json");

    let err = read(&path).unwrap_err();
    assert!(
        matches!(err, TreeStateError::LegacyManifestFound),
        "got: {err}"
    );
    // No state.json reconstructed.
    assert!(!path.exists());
}

#[test]
fn read_rejects_legacy_manifest_even_when_state_present() {
    let dir = TempDir::new().unwrap();
    stage_legacy_manifest(dir.path(), true);
    let path = dir.path().join(".bo/state.json");
...
#[test]
fn write_rejects_legacy_manifest_and_writes_nothing() {
    let dir = TempDir::new().unwrap();
    stage_legacy_manifest(dir.path(), false);
    let path = dir.path().join(".bo/state.json");

    let err = write(&path, &sample_state()).unwrap_err();
    assert!(
        matches!(err, TreeStateError::LegacyManifestFound),
        "got: {err}"
    );
    // write() must not create state.json or its tmp file.
    assert!(!path.exists());
    assert!(!PathBuf::from(format!("{}.tmp", path.display())).exists());
}

#[test]
fn load_state_propagates_legacy_manifest_found() {
    let dir = TempDir::new().unwrap();
    stage_legacy_manifest(dir.path(), false);

    // LegacyManifestFound must surface as an error, never map to MissingState.
...
#[test]
fn load_state_propagates_legacy_manifest_found_with_state_present() {
    let dir = TempDir::new().unwrap();
    stage_legacy_manifest(dir.path(), true);
...
fn runtime_state_distinguishes_fresh_missing_and_initialized() {
    ...
    write(
        &crate::domain::tree::state_path(dir.path()),
        &tree.empty_state(),
    )
    .unwrap();
    assert!(matches!(
...
#[test]
fn fresh_state_uses_tree_metadata() {
    let dir = TempDir::new().unwrap();
    let tree = Tree::from_config(&temp_config(&dir));

    let state = crate::engine::state::state_or_empty_if_fresh(&tree).unwrap();

    assert_eq!(state.tree.name, "my-research");
    assert_eq!(state.tree.created_at, timestamp());
