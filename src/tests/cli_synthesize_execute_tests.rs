...
#[test]
fn ensure_synthesis_context_fits_errors_on_overflow() {
    let model = Model::parse("gpt-4.1-mini", Provider::OpenAI).unwrap();

    let small = execute_prompt_tokens(64);
    assert!(ensure_synthesis_context_fits(&model, small).is_ok());

    let huge = execute_prompt_tokens(usize::MAX);
    assert!(
        ensure_synthesis_context_fits(&model, huge).is_err(),
        "overflow must error"
    );
}

/// Wrap a byte count into a token estimate comparable to what the synthesis
/// pipeline computes, so tests exercise the same fit-check path.
fn execute_prompt_tokens(prompt_bytes: usize) -> usize {
    estimate_synthesis_prompt_tokens(prompt_bytes)
}
