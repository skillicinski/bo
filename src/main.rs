...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some("agent") => match agent::run(args) {
            Ok(()) => {}
            Err(error) => {
...
fn seed(mut args: impl Iterator<Item = String>) -> Result<PathBuf, String> {
    let mut name = None;
...
fn state(mut args: impl Iterator<Item = String>) -> Result<(), String> {
    let name = args
        .next()
    ...
    if full {
        ...
        println!("{} documents snapped", records.len());
    }
    Ok(())
}

fn snap(mut args: impl Iterator<Item = String>) -> Result<usize, String> {
    let name = args
        .next()
...
fn snap_at(target: &Path, urls: &[String]) -> Result<usize, String> {
    ...
    for url in urls {
        match snap_one(&client, target, url) {
            Ok(snapshot) => {
                ...
                println!("snapped: {url} -> {}", snapshot.filename);
                succeeded += 1;
            }
            Err(error) => {
