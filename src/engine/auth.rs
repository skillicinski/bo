...
/// 1. Check env var (OPENAI_API_KEY, DEEPSEEK_API_KEY, GEMINI_API_KEY, ZAI_API_KEY, or CUSTOM_API_KEY)
/// 2. Fall back to ~/.bo/auth.json with flat keys `openai_api_key` / `deepseek_api_key` / `google_api_key` / `zai_api_key` / `custom_api_key`
