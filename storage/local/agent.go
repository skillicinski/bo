...
func RunAgent(ctx context.Context, rootPath, targetPath string, storage bo.Storage, provider bo.CompletionProvider, config bo.AgentConfig) (int, error) {
	if provider == nil {
		return 0, bo.RequestError("agent provider is not configured")
	}
	...
	for {
		...
		turns++
		response, err := provider.Complete(runContext, bo.CompletionRequest{
			Messages: messages, Tools: AgentTools(), ToolChoice: "auto", Stream: false,
		...
		if len(message.ToolCalls) > 0 {
			...
			for _, call := range message.ToolCalls {
				...
				messages = append(messages, bo.ChatMessage{Role: "tool", ToolCallID: call.ID, Content: boundedOutput(result, config.MaxToolOutputBytes)})
			}
			continue
		}
		...
		correctionSent = true
		messages = append(messages, message, bo.ChatMessage{Role: "user", Content: fmt.Sprintf("You stopped before completing the task. Use the bounded tools now and write successful summaries for every missing source identity: %s", strings.Join(missing, ", "))})
	}
}

func normalizedAgentConfig(config bo.AgentConfig) bo.AgentConfig {
	defaults := bo.DefaultAgentConfig()
...
func AgentTools() []bo.ToolDefinition {
	objectParameters := func(properties map[string]any, required []string) map[string]any {
		return map[string]any{"type": "object", "properties": properties, "required": required, "additionalProperties": false}
	}
	return []bo.ToolDefinition{
		{Type: "function", Function: bo.ToolDeclaration{Name: "bash", Description: "Run one bounded facade command: ls [path], cd path, cat raw.md, or grep literal [path]. This is not a shell.", Parameters: objectParameters(map[string]any{"command": map[string]any{"type": "string"}}, []string{"command"})}},
		{Type: "function", Function: bo.ToolDeclaration{Name: "read_state", Description: "Read the authoritative state for the target directory.", Parameters: objectParameters(map[string]any{}, []string{})}},
		{Type: "function", Function: bo.ToolDeclaration{Name: "read_summary", Description: "Read the existing Markdown summary for one source identity.", Parameters: objectParameters(map[string]any{"source_key": map[string]any{"type": "string"}}, []string{"source_key"})}},
		{Type: "function", Function: bo.ToolDeclaration{Name: "write_summary", Description: "Write or replace the Markdown summary for one source identity using its newest raw snapshot.", Parameters: objectParameters(map[string]any{"source_key": map[string]any{"type": "string"}, "markdown": map[string]any{"type": "string"}}, []string{"source_key", "markdown"})}},
	}
}
...
func sourceGroups(documents map[string]string, state bo.State) map[string]agentSource {
	names := make([]string, 0, len(documents))
	for name := range documents {
		names = append(names, name)
	}
...
func executeToolCall(context *agentContext, call bo.ToolCall, summarized map[string]bool) (string, error) {
...
func readSummary(context *agentContext, sourceKey string) (string, error) {
	...
	data, err := context.storage.ReadDocument(context.ctx, bo.SummaryRef(record.Filename))
	if err != nil {
		return "", err
	}
	return boundedOutput(string(data), context.maxOutputBytes), nil
...
func writeSummary(context *agentContext, sourceKey, markdown string) error {
	...
	if err := context.storage.ReplaceSummary(context.ctx, bo.SummaryRef(filename), []byte(markdown)); err != nil {
		return err
	}
	now, err := nowNanos()
	if err != nil {
	...
	record := bo.SummaryRecord{Filename: filename, SourceKey: sourceKey, DerivedFrom: source.LatestFilename, CreatedAt: createdAt, UpdatedAt: updatedAt}
	if existing == nil {
		record.CreatedAt = now
	}
	next := context.state
	next.Summaries = append([]bo.SummaryRecord{}, context.state.Summaries...)
	replaced := false
