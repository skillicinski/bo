...
func (h *HTTP) Fetch(ctx context.Context, input string) (bo.Page, error) {
	...
	if match.Kind == YouTubeUnsupported {
		return bo.Page{}, bo.UnsupportedError("YouTube URL: " + match.Reason)
	}
	parsed, err := url.Parse(input)
	if err != nil {
		return bo.Page{}, bo.InputError(fmt.Sprintf("invalid URL: %v", err))
	}
	if parsed.Scheme != "http" && parsed.Scheme != "https" {
		return bo.Page{}, bo.InputError("URL scheme must be http or https")
	}
	client := h.client()
	...
	if err != nil {
		return bo.Page{}, bo.RequestError(fmt.Sprintf("request failed: %v", err))
	}
	...
	if !isHTMLContentType(contentType) {
		return bo.Page{}, bo.ContentError(fmt.Sprintf("not HTML (Content-Type: %s)", contentType))
	}
	body, err := io.ReadAll(response.Body)
	if err != nil {
		return bo.Page{}, bo.ContentError(fmt.Sprintf("reading response failed: %v", err))
	}
...
func ExtractHTML(input string) (bo.Page, error) {
	document, err := html.Parse(strings.NewReader(input))
	if err != nil {
		return bo.Page{}, bo.ContentError(fmt.Sprintf("HTML parsing failed: %v", err))
	}
	title := strings.Join(strings.Fields(textContent(findElement(document, "title"))), " ")
	if title == "" {
		return bo.Page{}, bo.ContentError("page has no title")
	}
	...
	for _, tag := range []string{"article", "main", "body"} {
		...
		if err := html.Render(&rendered, element); err != nil {
			return bo.Page{}, bo.ContentError(fmt.Sprintf("HTML rendering failed: %v", err))
		}
		markdown, err := htmltomarkdown.ConvertString(rendered.String())
		if err != nil {
			return bo.Page{}, bo.ContentError(fmt.Sprintf("HTML conversion failed: %v", err))
		}
	...
	return bo.Page{}, bo.ContentError("page has no readable content")
}
