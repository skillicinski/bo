...
// Validate checks the storage-neutral workspace state rules.
func (s State) Validate() error {
	_, err := internalState(s)
...
func (s State) ApplySnapshot(commit SnapshotCommit) (State, error) {
	...
	if err != nil {
		return State{}, publicError(err)
	}
	next, err := state.ApplySnapshot(internalSnapshotCommit(commit))
	if err != nil {
		return State{}, publicError(err)
	}
	result, err := publicState(next)
	return result, publicError(err)
}

// ApplySummary returns the state after a summary commit.
func (s State) ApplySummary(commit SummaryCommit) (State, error) {
	...
	next, err := state.ApplySummary(internalSummaryCommit(commit))
	if err != nil {
		return State{}, publicError(err)
	}
	result, err := publicState(next)
...
// ApplyDistillation returns the state after a distillation commit.
func (s State) ApplyDistillation(commit DistillationCommit) (State, error) {
	...
	next, err := state.ApplyDistillation(internalDistillationCommit(commit))
	if err != nil {
		return State{}, publicError(err)
	}
	result, err := publicState(next)
...
func ValidateDocumentKind(kind DocumentKind) error {
	return publicError(internaldomain.ValidateDocumentKind(internaldomain.DocumentKind(kind)))
}
...
func ValidateContentDigest(digest string) error {
	return publicError(internaldomain.ValidateContentDigest(digest))
}
...
// Validate checks a raw document commit.
func (commit SnapshotCommit) Validate() error {
	return publicError(internalSnapshotCommit(commit).Validate())
}

// Validate checks a summary commit.
func (commit SummaryCommit) Validate() error {
	return publicError(internalSummaryCommit(commit).Validate())
}

// Validate checks a distillation commit.
func (commit DistillationCommit) Validate() error {
	return publicError(internalDistillationCommit(commit).Validate())
}
...
// Normalize fills the operation fields generated by a workspace commit.
func (operation *Operation) Normalize() {
	if operation == nil {
		return
	}
	value := internalOperation(*operation)
	value.Normalize()
	*operation = publicOperation(value)
}

// Validate checks the durable operation contract.
func (operation Operation) Validate() error {
	err := internalOperation(operation).Validate()
	if err == nil {
		return nil
	}
	return publicError(internalerrors.Wrap(internalerrors.KindValidation, "invalid operation", err))
}

// ValidateSeed checks the operation used to create a workspace.
func (operation Operation) ValidateSeed() error {
	return publicError(internalOperation(operation).ValidateSeed())
}
